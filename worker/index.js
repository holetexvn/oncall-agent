...
const args = [
  ...
  "--output-format", "json",                              // việc 4: envelope có session_id + cost để log
...
const t0 = Date.now();
const r = spawnSync(cmd, finalArgs, { cwd: REPO_DIR, env: process.env, encoding: "utf8", maxBuffer: 50 * 1024 * 1024 });
const secs = ((Date.now() - t0) / 1000).toFixed(1);

const runFile = path.join(RUNS, `run-${incident.id}.json`);
let envelope = null, result = null;
try {
  envelope = JSON.parse(r.stdout);
  result = envelope.structured_output ?? (typeof envelope.result === "string" ? JSON.parse(envelope.result) : envelope.result);
} catch (e) {
  console.error("[worker] cannot parse claude output:", e.message, "\nstdout:", r.stdout?.slice(0, 2000), "\nstderr:", r.stderr?.slice(0, 2000));
}
